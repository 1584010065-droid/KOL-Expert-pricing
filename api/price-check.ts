...
function normalizeModelResult(result: any, context: any) {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!client) {
    return res.status(503).json({
      error: "缺少 OPENAI_API_KEY。请在 Vercel 环境变量中设置 DEEPSEEK_API_KEY 或 OPENAI_API_KEY。"
    });
  }

  const context = req.body?.context;
  if (!context) {
    return res.status(400).json({ error: "缺少 creator_pricing_context_json。" });
  }
  ...
  try {
    ...
    return res.json({ result, model, responseId: completion.id });
  } catch (error) {
